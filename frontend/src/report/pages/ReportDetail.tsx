					...
					<Card className="border-0 text-center shadow-sm">
						<CardContent className="flex flex-col items-center pt-8 pb-6">
							{(() => {
								...
								const cls =
									CATEGORY_CLASSES[report.category] ??
									'bg-slate-100 text-slate-600'
								return (
									<div
										className={`mb-3 flex size-[72px] items-center justify-center rounded-full ${cls}`}
									>
										<Icon size={30} />
									</div>
								)
							})()}
							<h5 className="mb-2 text-[15px] font-bold">{report.title}</h5>
							<Badge
								variant="outline"
								className={`mb-2 rounded-full px-3 py-1 ${CATEGORY_CLASSES[report.category] ?? 'border-slate-200 bg-slate-100 text-slate-600'}`}
							>
								{report.category}
							</Badge>
							<Badge
					...
					<div className="space-y-4">
						<Card className="border-0 shadow-sm">
							...
							<CardContent className="pt-5">
								<dl className="grid grid-cols-[9rem_1fr] gap-y-4">
									...
									<DetailRow
										label="Created"
										value={formatDate(report.created)}
									/>
								</dl>
							</CardContent>
						</Card>
						...
						<Card className="border-0 shadow-sm">
							...
							<CardContent className="pt-4">
								<p className="leading-relaxed text-[#444]">{report.summary}</p>
							</CardContent>
						</Card>
					</div>
				</div>
